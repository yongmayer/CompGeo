...
def goImages():
    ifile = "image"
    i = readDat("./MIG/"+ifile,nz,nx) # read big endian files (.dat)
    # for little endian files (.bin), use readLittleDat() instead
    plot2Image(i,sz,sx,None,ifile) # plot an image and save it as a .png file
    #display2(i) # only plot an image
    ifile = "imageLowCut"
    i = readDat("./MIG/"+ifile,nz,nx) # read big endian files (.dat)
    plot2Image(i,sz,sx,None,ifile) # plot an image and save it as a .png file
#############################################################################
...
def plot2Image(s,s1,s2,title=None,png=None):
  n1 = len(s[0])
  n2 = len(s)
  panel = panel2Data()
